...
Once you have setup a CLI profile and HALO identity, you can then create a registry address on the DXNS registry:

```bash
dx ns address create
```
...
```bash
dx ns balance increase --faucet https://node2.devnet.dxos.network/kube/faucet --account 5EpqhyY9AfHgmrqwFs7tFh3V89ktNamgTP3TWM5zgeQM8y7a
```
