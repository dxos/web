...
In order to be able to publish anything to the DXNS registry you need to create a developer account. We're going to setup a DXNS developer account and register a domain. This will require your [development environment to be setup](./dev-environment), so make sure you complete that first.
...
TODO if faucet step doesn't become automatic, then what is faucet?
...
```bash
dx dxns auction create example 1000000
```
...
```bash
dx dxns auction close example
dx dxns auction claim example
```
