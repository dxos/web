 ...
 */
export const SmallText = ({ children, variant = "dark", ...props }) => {
  if (!children) {
    return null;
  }
  const textColorClass = getTextColorClassForVariant(variant, 60);
  return (
    <div
      className={`dxos-body-text dxos-lh-very-loose dxos-text-wrap-break-word dxos-f1 ${textColorClass} variant-${variant}`}
